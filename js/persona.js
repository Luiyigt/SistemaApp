        ...
        "columns": [
          ...
          render: function(data, type, row) {
                if (data == 'On') {
                    ...
                    "&nbsp;<button type='button' class='desactivar btn btn-warning btn-sm'><i class='fa fa-eye-slash' aria-hidden='true'></i></button>";
                } else {
                    return "<button type='button' class='editar btn btn-primary btn-sm'><i class='fa fa-edit' title='editar'></i></button>";
                }
            }}
 ...
 var request = JSON.parse(result);
  ...
  $("#MonedaPersona").val(request[0]['Moneda']);
      ...
      // Obtener la extensión del archivo
           ...
           // Verificar si la extensión corresponde a PDF o Word
           if (extension === "pdf") {
            //button quitar cv visible
             $("#quitarArchivoButton").show();
             // Es un archivo PDF
             $("#nombreArchivoLabel").html('<iframe src="' + "../Files/" + encodeURIComponent(request[0]["cvpersona"]) + '"></iframe>');
           }  else {
            ...
            if(nombreFoto){
             ...
             var src =  "../Files/" + encodeURIComponent(nombreFoto);
...
if(validation()){
  ...
  var salario= $("#SalarioPersona").val();
  var sexo= $("#SexoPersona").val();
  var monedas= $("#MonedaPersona").val();
  var entrevista= $("#entrevistaPersona").val();
  var resulentrevistas= $("#resultentrevistaPersona").val();
 ...
 //Cargando el pdf o word
   var fileInput = document.getElementById('archivoInput');
    ...
    formData.append('monedas',monedas);
    formData.append('entrevista',entrevista);
    formData.append('resulentrevistas',resulentrevistas);
    ...
    success: function(respuesta){
       ...
       return Swal.fire("Mensaje de error", "No se pudo "+request.msg, "error");
      }
