         ...
         if (move_uploaded_file($rutaTempArchivo, $rutaDestino)) {
          ...
          if (isset($_FILES['foto']) && $_FILES['foto']['error'] === UPLOAD_ERR_OK) {
              ...
              $rutaTempfoto = $fotopersona ['tmp_name'];
                 ...
                 // Mover el archivo a la carpeta destino
                   if (move_uploaded_file($rutaTempfoto, $rutaDestinofoto)) {
                       // El archivo se ha movido exitosamente
                       // echo 'El archivo ' . $nombreArchivo . ' se ha subido correctamente.';
                        $nombrefotoedit = "IMG_".$nombres."_".$dni."_".$nombrefoto;
            ...
            $filename = isset($nombreArchivo) ? $nombreArchivo : "";
              $imgname = isset($nombrefotoedit) ?  $nombrefotoedit: "";

          


  if (!empty($imgname) || !empty($filename)) {
    // Al menos una de las variables está definida, puedes utilizarlas aquí
  
    if (!empty($imgname)) {
        
     
         $consulta = $persona->Registrar_PersonaConFoto($idPersona,$nombres,$apellidos,$correo,$dni,$telefono,$direccion,$salario,$sexo,$monedas,$entrevista,$resulentrevistas,$imgname);
    ...
    if (!empty($filename)) {
        ...
        $consulta = $persona->Registrar_PersonaConFile($idPersona,$nombres,$apellidos,$correo,$dni,$telefono,$direccion,$salario,$sexo,$monedas,$entrevista,$resulentrevistas,$filename);
        
       
        
    }
    
    $response = array('status' => true,'auth' => true,'msg' => 'La operacion se completo correctamente.','data'=>$consulta);
    echo json_encode($response);
    return;

} 

     $consulta = $persona->Registrar_PersonaSinNada($idPersona,$nombres,$apellidos,$correo,$dni,$telefono,$direccion,$salario,$sexo,$monedas,$entrevista,$resulentrevistas);
