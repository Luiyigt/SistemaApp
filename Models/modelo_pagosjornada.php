<?php
 ...
 function Listar_Personal_On(){
  ...
  $sql="SELECT idpersona,Nombre,Apellidos,Dni,Salario,FechaInicio,fechapago,EstadoCuenta FROM jornada
...
function PagosMensualesPersonal($mesSeleccionado,$fechaspagado, $monto,$idpersona,$dateoperation){
     
     $sql = "INSERT INTO pagosjornada (idpersona, montoP, Description, fechasPagados, fechaoperacion, adelantos,montoextra, stado)
                             VALUES ('$idpersona','$monto','Pago Jornada mensuale', '$fechaspagado','$dateoperation','0','0','PAGADO')";
          if ($consulta = $this->conexion->conexion->query($sql)) {
...
function ActualizarEstadoFornadaPersonalOn($idpersona,$fechapagado,$fechaproximo){
...
function PagarAdelantosPersonalOn($idpersona,$dateoperacion,$totaladelantos){

 $sql = "INSERT INTO pagosjornada (idpersona, montoP, Description, fechasPagados, fechaoperacion,adelantos, stado)
                 VALUES ('$idpersona','0','pago adelantos personales', '$dateoperacion','$dateoperacion','$totaladelantos','PAGADO')";
          if ($consulta = $this->conexion->conexion->query($sql)) {
...
function PagarHorasExtrasPersonalOn($idpersona,$dateoperacion,$totalextra){
     $sql = "INSERT INTO pagosjornada (idpersona, montoP, Description, fechasPagados, fechaoperacion,montoextra, stado)
                 VALUES ('$idpersona','0','Pago horas extras', '$dateoperacion','$dateoperacion','$totalextra','PAGADO')";
          if ($consulta = $this->conexion->conexion->query($sql)) {
...
function Filtrar_Reportes_Pagosjornada($fechaInicio,$fechaFin){
 $sql="select  personas.idpersona,Nombre, Apellidos,dni, montoP, Description, fechasPagados, fechaoperacion,adelantos FROM pagosjornada
...
function ComboSelectdDatePagos($idpersona) {
    $sql = "SELECT fechaoperacion FROM pagosjornada WHERE idpersona = '$idpersona'";
    $arreglo = array();
/*
    if ($consulta = $this->conexion->conexion->query($sql)) {
        while ($consulta_VU = mysqli_fetch_assoc($consulta)) {
            $fecha = $consulta_VU['fechaoperacion'];

            // Verificar si la fecha ya está en el arreglo
            if (!in_array($fecha, $arreglo)) {
                $arreglo[] = $fecha;
            }
        }
        $this->conexion->cerrar();
