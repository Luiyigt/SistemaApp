...
function Listar_Horas_Extras_Empleados() {
    $sql = "SELECT idepersona, Nombre, Apellidos, Dni, Salario, fecharegistro, hextra, total, stadohoral 
FROM horaextra
    ...
    if ($consulta = $this->conexion->conexion->query($sql)) {
        while ($consulta_VU = mysqli_fetch_assoc($consulta)) {
            ...
            if (!in_array($valorUnico, $valoresUnicos)) {
               ...
               //$arreglo["data"][$indice]['hextra'] = number_format($arreglo["data"][$indice]['hextra'], 2);
            }
        }
...
function Registrar_Pagos_Horas_Extras($idextra,$fecha,$horastotal,$montototal,$idpersona,$fechaActual){
 ...
 $sql = "INSERT INTO pagoshextra (idhextra, fechas, horas, monto, idpersona, datecreated, descripcion,stated) 
      VALUES ('$idextra','$fecha','$horastotal','$montototal','$idpersona','$fechaActual','Concept.H.EXTRA','Pagado')";
         if ($consulta = $this->conexion->conexion->query($sql)) {
...
/*
function DarDeBajasHorasExtras($idextra,$idempleado) {
   $sql = "update h_extra set stadohoral ='Pagado'
     where idh_extra ='$idextra' and idemplead ='$idempleado'";
           if ($consulta = $this->conexion->conexion->query($sql)) {      
              return 1;
            }else{
              return 0;
